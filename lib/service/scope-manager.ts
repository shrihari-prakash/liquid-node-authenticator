...
class ScopeManager {
  ...
  async initializeScopes(): Promise<void> {
    try {
      ...
      const scopeCount = Object.keys(this.scopes).length;
      this.logger.info(
        `Scopes initialized. ${scopeCount} scope(s) loaded from ${this.host}`,
      );
      if (scopeCount === 0) {
        this.logger.warn(
          "No scopes were loaded. Scope checks will always deny access.",
        );
      }
    } catch (error: any) {
  ...
  isScopeAllowed(scope: string, allowedScopes: string[] = []): boolean {
    ...
    if (!scopeObject) {
      this.logger.warn(
        `Unknown scope ${scope}. Did you forget to configure this scope in your Liquid server?`,
      );
      return false;
    }
    if (
      allowedScopes.includes(scopeObject.name) ||
      (scopeObject.parent && allowedScopes.includes(scopeObject.parent))
    ) {
      return true;
    } else if (scopeObject.parent) {
      this.logger.debug(
        `Scope "${scope}" not directly granted, checking parent scope "${scopeObject.parent}"`,
      );
